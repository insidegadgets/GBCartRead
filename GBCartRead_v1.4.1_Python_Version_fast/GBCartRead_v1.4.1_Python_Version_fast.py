...
ser = serial.Serial('/dev/cu.usbmodem1411', 460800) # Change COM2 to the port the Arduino is on
...
while (waitInput == 1):
    ...
    if (userInput == "1"):      
        ...
        dat = bytearray(siz*1024);
        blocksremaining=siz;
        while blocksremaining > 0:
            if not(inRead):
                lineascii = ascii(ser.readline())
                ...
                if (siz-blocksremaining) % 50 == 0:
                    print(str(siz-blocksremaining) + "K",end="",flush=True);
                if (siz-blocksremaining) % 250 == 0:
                    print(" " + str(time.time() - ts),end="",flush=True)
                    ts=time.time()
        sys.stdout.write('\nFinished\n\n')
