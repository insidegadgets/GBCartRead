...
int latchPin = 10; // NEEDS TO BE PIN 10
int dataPin = 11; // NEEDS TO BE PIN 11
int clockPin = 13; // NEEDS TO BE PIN 13
int rdPin = A5;
int wrPin = A3;
int mreqPin = A4;

int dataMode = -1; // -1 means unset, 0 for read, 1 for write
...
void setup() {
  ...
  // set up SPI for shift registers
  SPI.begin();
  ...
  // Dump ROM
    ...
    for (int y = 1; y < romBanks; y++) {
      ...
      for (; addr < 0x8000; addr++) {
        ...
        serDataBuf[bufcount % 1024] = readdataPins(); // Read data
        digitalWrite(rdPin, HIGH); // RD off 
        bufcount++;
        if(bufcount % 1024 == 0) {
          Serial.write(serDataBuf, 1024);
          bufcount = 0;
        }
      }
    }
...
void setdataPinMode(int mode) {
   // Change to inputs
  if(dataMode != 0 && mode == 0) {
    for (int i = 2; i <= 9; i++) {
      pinMode(i, INPUT);
    }
    dataMode = 0;
  }
  else if(dataMode != 1 && mode == 1) {
      // Change to outputs
    for (int i = 2; i <= 9; i++) {
      pinMode(i, OUTPUT);
    }
...
// Read data pins
byte readdataPins() {
  // set the data pins to input if necessary
  if(dataMode != 0) {
    setdataPinMode(0);
  }
  // Read data pins
  byte bval = 0;
  bval = ((PINB & 0x3) << 6) | (PIND >> 2); // ultra fast for uno only
  //bval = ((digitalReadFast2(9)<<7) | (digitalReadFast2(8)<<6) | (digitalReadFast2(7)<<5) | (digitalReadFast2(6)<<4)
  //      | (digitalReadFast2(5)<<3) | (digitalReadFast2(4)<<2) | (digitalReadFast2(3)<<1) | digitalReadFast2(2));
  /*
  for (int z = 9; z >= 2; z--) {
